    ...
    <div className="min-h-screen flex flex-col bg-night-900 text-cream-50">
      <header className="border-b border-cream-50/[0.06]">
        <div className="max-w-7xl mx-auto px-5 sm:px-6 py-4 flex items-center justify-between gap-4">
          ...
          <Link to="/" className="text-[10px] uppercase tracking-[0.22em] text-cream-50/55 hover:text-gold-100 transition">
            ← Back
          </Link>
        </div>
      </header>
      <main className="flex-1 flex items-start justify-center px-5 sm:px-6 py-12">
        <div className="w-full max-w-md">
          ...
          {subtitle && <p className="text-cream-50/55 text-sm mb-10">{subtitle}</p>}
          {children}
        </div>
      </main>
      <footer className="px-6 py-8 text-center text-[10px] tracking-[0.28em] uppercase text-gold-900">
